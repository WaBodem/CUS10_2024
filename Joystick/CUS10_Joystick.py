...
def init_joystick():
    ...
    for channel, avg in averages.items():
        ...
        if scale_voltage_to_10bit(avg,cfg.ADC_REF) > 512+channel_names[channel]['zulkaliboffset'] or scale_voltage_to_10bit(avg,cfg.ADC_REF) < 512-+channel_names[channel]['zulkaliboffset']l:
...
def main(): 
    ...
    try:
        ...
        client.publish(cfg.MQTT_ID+cfg.JS_TOPIC+cfg.OS_TOPIC, "Joystick Task gestartet")
            ...
            while(1):
                ...
                if time() > mqtt_JoystickpubTimer and bJoystickInit:
                    ...
                    if sum(VR150_liste_channel_values) != 512*6 or not bJoystickStop_aftermove:
                        client.publish(cfg.MQTT_ID+cfg.JS_TOPIC+cfg.JOYSTICKVAL_TOPIC+cfg.JOYSTICKMODBUSVR150_TOPIC, str(VR150_liste_channel_values[0:6]))
                ...
                if time() > mqtt_JoystickRawpubTimer and not bJoystickInit:
                    mqtt_JoystickRawpubTimer = time() + cfg.MQTT_JoystickRawTimer
                    for i, channel in enumerate(channelList):
                            client.publish(cfg.MQTT_ID + cfg.JS_TOPIC + cfg.JOYSTICKVAL_TOPIC + '/CH' + str(channel_names[channel]),
